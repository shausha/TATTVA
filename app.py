...
ALLOWED_EXTENSIONS =from flask import Flask, request, render_template, redirect, url_for
import sqlite3
from lxml import etree
from werkzeug.utils import secure_filename
import os
...
def upload():
    if request.method == "POST":
        ...
        # Read and decode the text file using latin-1 encoding
        try:
            text_content = text_file.read().decode("latin-1")
...
def create_tei(text_content):
    ...
    author_elem = etree.SubElement(title_page, "p")
    author_elem.text = "Author: Unknown"
