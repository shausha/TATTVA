...
def upload():
    if request.method == "POST":
        ...
        query_database(
            "INSERT INTO Metadata (title, author, year, text, pdf) VALUES (?, ?, ?, ?, ?)",
            (title, author, year, tei_content, pdf_content)
        )
...
def book(book_id):
    # Fetch book metadata and PDF
    metadata = query_database("SELECT title, author, year, text FROM Metadata WHERE book_id = ?", (book_id,))
    pdf_content = query_database("SELECT pdf FROM Metadata WHERE book_id = ?", (book_id,))
    if not metadata or not pdf_content or not pdf_content[0][0]:
        return "Book or PDF not found.", 404

    # Extract metadata fields
    title, author, year, text = metadata[0]
    pdf_binary = pdf_content[0][0]

    # Pass these fields to the template
    return render_template("book.html", title=title, author=author, year=year, text=text, pdf_binary=pdf_binary, book_id=book_id)

@app.route("/book/<int:book_id>/pdf")
def view_pdf(book_id):
    pdf_content = query_database("SELECT pdf FROM Metadata WHERE book_id = ?", (book_id,))
    if not pdf_content or not pdf_content[0][0]:
        return "PDF not found.", 404

    pdf_binary = pdf_content[0][0]
    return Response(pdf_binary, mimetype="application/pdf")
